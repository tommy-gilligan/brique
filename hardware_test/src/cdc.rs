...
impl CdcTest<'_> {
    pub fn new() -> Self {
        Self(
            ...
            shared::confirmation::Confirmation::new("Perform optional test of USB CDC?", "Yes", "No", false),
            false,
            false
        )
    }
    ...
    pub async fn run(&mut self, device: &mut impl shared::Device, system_response: Option<[u8; 64]>) -> Status {
        match (self.1.run(device).await, self.2, self.3) {
            ...
            (Some(true), false, _) => {
                ...
                Status::InProgress(Some(shared::SystemRequest::UsbTx(shared::UsbTx::CdcBuffer(buf))))
            }
